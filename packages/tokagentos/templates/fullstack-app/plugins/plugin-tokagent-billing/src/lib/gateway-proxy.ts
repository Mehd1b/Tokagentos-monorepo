/**
 * Typed HTTPS client for the hosted Tokagent gateway (v2.0.0 — client-mode).
 *
 * In `BILLING_MODE=client`, every plugin route forwards through one of the
 * methods below to an upstream tokagentos instance running in `server` mode.
 * The wire contract mirrors the §3 endpoints documented in MIGRATION_PLAN.md —
 * any drift between this client and the gateway is a regression.
 *
 * Design notes:
 *   - Auth headers (`Authorization`, `x-api-key`) are passed through as-is.
 *     The client-mode CLI does NOT decode tokens or mint API keys locally.
 ...
 *     from `BillingConfig.gatewayUrl` (set when BILLING_MODE=client and
 *     TOKAGENT_GATEWAY_URL is configured).
 */
...
function pickForwardHeaders(
  ...
  if (headers["anthropic-version"])
    out["anthropic-version"] = headers["anthropic-version"];
...
/**
 * Convenience grouping used by route files. Each property maps a plugin
 * route to its corresponding gateway client method. Lets route files write
 * `state.gateway.auth.nonce(...)` rather than memorizing flat client names.
 */
export interface GatewayProxy {
  client: GatewayClient;
  auth: {
    nonce: (body: { wallet: string }) => Promise<ProxyResponse>;
    nonceGet: (query: {
      wallet: string;
      chainId?: number;
    }) => Promise<ProxyResponse>;
    login: (body: unknown) => Promise<ProxyResponse>;
    status: () => Promise<ProxyResponse>;
  };
  keys: {
    list: (headers: ForwardHeaders) => Promise<ProxyResponse>;
    create: (
      headers: ForwardHeaders,
      body: { name?: string },
    ) => Promise<ProxyResponse>;
    delete: (headers: ForwardHeaders, id: string) => Promise<ProxyResponse>;
  };
  credits: {
...
export function createGatewayClient(opts: GatewayProxyOptions): GatewayClient {
  ...
  return {
    ...
    authNoncePost: (body) => request("POST", "/v1/auth/nonce", {}, body),
    authNonceGet: (query) =>
    ...
    usageSummary: (headers, query) =>
      request(
        "GET",
        `/v1/usage/summary${buildQuery(query)}`,
        pickForwardHeaders(headers),
        undefined,
      ),
    usageCalls: (headers, query) =>
      request(
        "GET",
        `/v1/usage/calls${buildQuery(query)}`,
        pickForwardHeaders(headers),
        undefined,
      ),
    usageKeys: (headers, query) =>
      request(
        "GET",
        `/v1/usage/keys${buildQuery(query)}`,
        pickForwardHeaders(headers),
        undefined,
      ),
...
// ---------------------------------------------------------------------------
// GatewayProxy grouping factory
// ---------------------------------------------------------------------------

/**
 * Build the route-friendly GatewayProxy from a base GatewayClient. The
 * grouped property names mirror the plugin's route files for readability.
 */
export function createGatewayProxy(opts: GatewayProxyOptions): GatewayProxy {
  const client = createGatewayClient(opts);
  return {
    client,
    auth: {
      nonce: (body) => client.authNoncePost(body),
      nonceGet: (query) => client.authNonceGet(query),
      login: (body) => client.authLogin(body),
      status: () => client.billingStatus(),
    },
    keys: {
      list: (headers) => client.keysList(headers),
      create: (headers, body) => client.keysCreate(headers, body),
      delete: (headers, id) => client.keysDelete(headers, id),
    },
    credits: {
    ...
    topup: {
      ...
      settle: (headers, body) => client.topupSettle(headers, body),
      preauth: (headers, body) => client.topupPreauth(headers, body),
      status: (headers) => client.topupStatus(headers),
      revoke: (headers, body) => client.topupRevoke(headers, body),
      quoteGet: (id) => client.quoteGet(id),
    },
    usage: {
      summary: (headers, query) => client.usageSummary(headers, query),
      calls: (headers, query) => client.usageCalls(headers, query),
      keys: (headers, query) => client.usageKeys(headers, query),
      stats: () => client.stats(),
    },
    estimate: {
